...
export default function BiTffPage() {
  ...
  useEffect(() => {
    // Buscar o token do Metabase para dashboard 2 (TFF)
    fetch('/api/metabase/token?dashboard=2')
      .then(res => res.json())
        ...
        <header className="bg-white/95 backdrop-blur-xl border border-gray-200 shadow-lg m-4 p-4 md:p-6 rounded-2xl">
          <div className="flex flex-col md:flex-row justify-between items-center gap-4">
            <div className="flex items-center gap-8">
              <div className="relative w-32 h-20 cursor-pointer" onClick={() => router.push('/dashboard')}>
                <Image
                  src="/logo.png"
                  alt="ITIV Logo"
                  fill
